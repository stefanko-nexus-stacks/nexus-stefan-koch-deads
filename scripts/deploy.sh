...
SSH_HOST="ssh-stefan-koch-deads.nona.company"
...
KESTRA_URL=https://kestra-stefan-koch-deads.nona.company
EOF
...
CB_SERVER_URL=https://cloudbeaver-stefan-koch-deads.nona.company
...
REDIRECT_URL=https://hoppscotch-stefan-koch-deads.nona.company
WHITELISTED_ORIGINS=https://hoppscotch-stefan-koch-deads.nona.company
VITE_BASE_URL=https://hoppscotch-stefan-koch-deads.nona.company
VITE_SHORTCODE_BASE_URL=https://hoppscotch-stefan-koch-deads.nona.company
VITE_ADMIN_URL=https://hoppscotch-stefan-koch-deads.nona.company/admin
VITE_BACKEND_GQL_URL=https://hoppscotch-stefan-koch-deads.nona.company/backend/graphql
VITE_BACKEND_WS_URL=wss://hoppscotch-stefan-koch-deads.nona.company/backend/graphql
VITE_BACKEND_API_URL=https://hoppscotch-stefan-koch-deads.nona.company/backend/v1
...
PREFECT_UI_API_URL=https://prefect-stefan-koch-deads.nona.company/api
EOF
...
LAKEFS_GATEWAYS_S3_DOMAIN_NAME=s3.lakefs-stefan-koch-deads.nona.company
...
APPSMITH_CUSTOM_DOMAIN=https://appsmith-stefan-koch-deads.nona.company
EOF
...
NC_PUBLIC_URL=https://nocodb-stefan-koch-deads.nona.company
...
EOF
    ...
    if [ -n "$INFISICAL_TOKEN" ] && [ -n "$PROJECT_ID" ]; then
        ...
        build_folder "gitea" \
            ...
            "GITEA_REPO_URL" "https://git-stefan-koch-deads.nona.company/${GITEA_REPO_OWNER:-$ADMIN_USERNAME}/${REPO_NAME:-nexus-${DOMAIN//./-}-gitea}.git" \
            "GITEA_DB_PASSWORD" "$GITEA_DB_PASS"
    ...
    cron: \"*/15 * * * *\"'" >/dev/null 2>&1 || true
            ...
            if echo "$ENABLED_SERVICES" | grep -qw "woodpecker" && [ -n "$WOODPECKER_AGENT_SECRET" ]; then
                ...
                OAUTH_RESULT=$(ssh nexus "curl -s -X POST 'http://localhost:3200/api/v1/user/applications/oauth2' \
                    ...
                    -d '{
                        \"name\": \"Woodpecker CI\",
                        \"redirect_uris\": [\"https://woodpecker-stefan-koch-deads.nona.company/authorize\"],
...
if echo "$ENABLED_SERVICES" | grep -qw "wikijs" && [ -n "$WIKIJS_ADMIN_PASS" ]; then
    (
        ...
        SETUP_PAYLOAD=$(jq -n \
            ...
            --arg url "https://wiki-stefan-koch-deads.nona.company" \
